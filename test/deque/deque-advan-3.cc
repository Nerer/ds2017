...
bool check1(){ // test push_back push_front pop_back pop_front
    ...
    for(int i = 0; i < 3000; i++) if(Q[i] != stdQ[i]) return 0;
...
bool check3(){ // copy && "=" && clear()
    ...
    sjtu::deque<int> Q1(Q);
    for(int i = 2999; i >= 0; i--) if(Q1[i] != stdQ[i]) return 0;
    it = Q1.end() - 1;
    for(stdit = stdQ.end() - 1; stdit != stdQ.begin(); stdit--){
        if(*stdit != *it) return 0;
        it--;
    ...
    sjtu::deque<int> P;
    P = Q1; P = Q1;
    for(int i = 2999; i >= 0; i--) if(P[i] != stdQ[i]) return 0;
...
bool check4(){ // test const_iterator at()
    ...
    for(int i = 0; i < 3000; i++) if(stdQ[i] != Q.at(i)) return 0;
    return 1;
}
...
bool check5(){ // test class writen by users
    ...
    for(int i = 0; i < 3000; i++) if(Q[i].num != stdQ[i]) return 0;
